...
> Từ bảng so sánh nhóm: cùng LocalEmbedder nhưng kết quả dao động 3/5 → 5/5 chỉ vì khác chunking/pipeline — chứng tỏ **không thể chỉ nhìn score cosine cao**; cần chuẩn hóa snapshot corpus, filter, top_k và tiêu chí evidence. ArticleChunker của Mạnh đạt 4/5 với ít vector hơn (379) nhờ giữ trọn một Điều, gợi ý hướng tốt cho corpus quy định; còn Q5 cho thấy filter `audience=student` đúng phạm vi nhưng không đủ — cần thêm metadata `institution`/`program_type` hoặc rerank theo cụm "ELITECH/CTĐT Tài năng".
